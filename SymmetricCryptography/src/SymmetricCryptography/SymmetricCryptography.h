...
class SymmetricCryptography {
    ...
    vector<vector<unsigned char>> H_table {
    ...
    ///Writes data to file
    void Write();
    ///Splits key to 8 32-bit(4 bytes) values
    vector<uint32_t> GetKeyNumbers();
    ///Modular plus
    uint32_t ModularPlus(uint32_t a, uint32_t b);
    ///Modular minus
    uint32_t ModularMinus(uint32_t a, uint32_t b);
    ///Cyclic shift to left for n bits and then to right for 32-n bits
    uint32_t CyclicShift(uint32_t value, short n);
    ///G-transform
    uint32_t GTransform(uint32_t value, short n);
