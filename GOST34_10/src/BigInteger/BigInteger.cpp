//
// Created by Grimlock on 15.11.2023.
//

#include "BigInteger.h"

bool BigInteger::is_bigint(std::string s) {                              // Checks if the feeded integer is valid Number or not.
    if(s[0] == '-')
        s.erase(0, 1);
...
std::string BigInteger::add(std::string str1, std::string str2) {          // returns arithmetic addition of str1+str2
...
std::string BigInteger::subtract(std::string str1, std::string str2) {                 // returns arithmetic subtraction of str1-str2
    int str1_len = str1.length();
    int str2_len = str2.length();
    std::string sum = "";
    if(str1 == str2) {
        ...
        if(str2_len == 1 && str1_len == 1) {
            ...
            BigInteger t1(str1.erase(0, 1));
            BigInteger t2(str2.erase(0, 1));
            std::string temp = subtract(t2.str, t1.str);
            BigInteger mx = maximum(t2.str, t1.str);
        ...
        if(str1_len < str2_len) {
            ...
            BigInteger a(str1);
            BigInteger b(str2);
            BigInteger mx = maximum(a.str, b.str);
            if(mx.str == str2) {
                std::string tp = str1;
                str1 = str2;
                str2 = tp;
                BigInteger t1(str1);
                BigInteger t2(str2);
                BigInteger temp = subtract(str1, str2);
                sum = "-" + subtract(str1, str2);
                return trim(sum);
            }
        }
...
std::string BigInteger::multiply(std::string str1, std::string str2) {             // return arithmetic multiplication of str1*str2
    bool toAddNeg = false;
    int str1_len = str1.length();
    int str2_len = str2.length();
    std::string ans = "";
...
std::string BigInteger::divide(std::string str1, std::string str2) {                   // return arithmetic division of str1/str2
    std::string ans = "";
    if(str2 == "0") {
        return "0";
    } else if(str1 == str2) {
...
std::string BigInteger::shortDivide(std::string s1, unsigned long long int divisor) {     // return arithmetic division of str1/str2
    std::string ans;
    int idx = 0;
...
std::string BigInteger::mod(std::string str1, std::string str2) {                  // return arithmetic modulos of str1%str2
    std::string ans = subtract(str1, multiply(divide(str1, str2), str2));
    return ans;
}


std::string BigInteger::maximum(std::string str1, std::string str2) {              // return maximum of both strings.
    std::string max = "";
    bool bothNeg = false;
    bool isMax1 = false;
    bool isMax2 = false;
...
std::string BigInteger::minimum(std::string str1, std::string str2) {              // return minimum of both strings.
...
bool BigInteger::is_maximum(std::string str1, std::string str2) {             //checks if str1 >= str2, numerically
    if(str1 == maximum(str1, str2))
        return true;
    return false;
}

bool BigInteger::is_minimum(std::string str1, std::string str2) {             //checks if str1 <= str2, numerically
    if(str2 == maximum(str1, str2))
        return true;
    return false;
}

bool BigInteger::is_strictlyMaximum(std::string str1, std::string str2) {    //checks if str1 > str2, numerically
    if(str1 == str2)
        return false;
    if(str1 == maximum(str1, str2))
        return true;
...
bool BigInteger::is_strictlyMinimum(std::string str1, std::string str2) {    //checks if str1 < str2, numerically
    if(str1 == str2)
        return false;
    if(str2 == maximum(str1, str2))
        return true;
...
std::string BigInteger::trim(std::string s) {                         // function to remove zeros
    if(s == "0")
        return s;
    if(s[0] == '-') {
        int i = 1;
...
std::string BigInteger::abs(std::string s) {                          // returns absolute value of string
    if(s[0] == '-')
        s.erase(0, 1);
    return s;
}
...
std::string BigInteger::pow(std::string str1, std::string str2) {                  // returns str1^str2, ^ -> power, numerically
    if(str2 == "0") {
        return "1";
    } else if(str1 == "0") {
        if(str2[0] == '-')
...
std::string BigInteger::sqrt(std::string s) {                 // returns sqrt(s), numerically
    if(s[0] == '-')
        return s;
    if(s == "0")
        return "0";
...
std::string BigInteger::log2(std::string s) {                 // returns log(s) to base of 2
    if(s == "0") {
...
std::string BigInteger::log10(std::string s) {                // returns log(s) to base of 10
    if(s == "0") {
        // throw new exception(std::out_of_range("log(0) is undefined"));
        return std::to_string(std::log2(0));
    }
...
std::string BigInteger::logwithbase(std::string val, std::string base) {
    return divide(log2(val), log2(base));
}

std::string BigInteger::antilog2(std::string s) {
    return pow("2", s);
}

std::string BigInteger::antilog10(std::string s) {
    return pow("10", s);
}

void BigInteger::swap(std::string& str1, std::string& str2) {
    std::string s;
    s = str1;
    str1 = str2;
    str2 = s;
...
std::string BigInteger::reverse(std::string s) {          // reverses the string number, example: "7875" => "5787"
    bool fl = false;
    if(s[0] == '-') {
        s.erase(0, 1);
        fl = true;
...
std::string BigInteger::gcd(std::string str1, std::string str2) {
    if(is_strictlyMaximum(str2, str1))
        swap(str1, str2);
    std::string temp = "";
...
std::string BigInteger::lcm(std::string str1, std::string str2) {      // return lcm of both str1 and str2
    return divide(multiply(str1, str2), gcd(str1, str2));
}

std::string BigInteger::fact(std::string s) {                     // returns the factorial of string s, numerically
    if(s[0] == '-') {
...
bool BigInteger::isPalindrome(std::string s) {                  // checks if the Integer as string, s is Palindrome no. or not
    if(s[0] == '-')
        s.erase(0, 1);
    unsigned long long int beg = 0, end = s.length() - 1;
    while(beg < end) {
...
bool BigInteger::isPrime(std::string s) {                // checks if the String as Number is prime or not
    if(maximum(s, "2") != s)
        return false;
    std::string sqrt = BigInteger::sqrt(s);
    std::string i = "";
